...
def compute_terms(file_full, file_out, file_global='means.nc'):
    ...
    for coord in ('time', 'thlev', 'lat', 'lon', 'plev_bnds'):
    ...
    dx = a * np.pi * (lon[1] - lon[0]) / 180.0  # radians times average Earth radius
    ...
    # NOTE: SLP could be scalar (case of interpolation from isobars), or array
    # (interpolation from hybrid coords).
    p = data_full['p'][:]
    u = data_full['u'][:]
    v = data_full['v'][:]
    z = data_full['z'][:]
    ...
    # Pseudo density
    # Is -(1 / g) * dp / dtheta; we apply scalar corrections to vector for efficiency
    ...
    sigma = -climo.deriv_uneven(0.01 * thlev * g, p, False, 0, 1)  # mass weighting
    ...
    vor = metpy.potential_vorticity(u, v, lat, lon)
    pv = cor + vor
    ...
    # z_star = z - z_bar
    # dzdx_star = climo.deriv_uneven(dx, z_star, axis=3)  # TODO: add cyclic option
    ...
    m_star = m - m_bar
    dmdx_star = climo.deriv_uneven(dx, m_star, axis=3)  # TODO: add cyclic option
    stress = 100.0 * weighted_mean(p_star * dmdx_star) / g
    make_variable(
