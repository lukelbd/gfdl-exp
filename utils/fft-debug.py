#!/usr/bin/env python3
#------------------------------------------------------------------------------#
# Copy of 'spectra.py' from when we were trying to debug (1) issues with 
# strength of power in transformed data (wasn't normalizing by length of
# the array), (2) issues with power reduction when using windows (occurs
# when using any non-boxcar window, and has no standard power reduction
# factor other than perhaps forcing the variance to match the sum of square
# Fourier coefficients), and (3) understanding whether marginal sum of doubly
# transformed data equals the average of 1D transforms across the 2nd transform
# dimension (which seems to be approximately true; although get slight power
# reduction on order of 1%; could not figure out exactly where this came from,
# but probably due to shifting of the *mean* in time; very small effect).
# The 1% error doesn't show up on singly (1D) transformed data, only doubly
# transformed data.
#------------------------------------------------------------------------------#
# Imports
...
paramsstar = {'ehf': (tstarstar, vstarstar),
          'emf':     (ustarstar, vstarstar),
          'eqgpvf':  (pvstarstar, vstarstar),
          }
paramsstar2 = {'ehf': (tstarstar, vstarstar),
          'emf':     (ustarstar, vstarstar),
          'eqgpvf':  (pvstarstar, vstarstar),
          }
# Potentially load
...
for flux in ('ehf','emf','eqgpvf'):
    ...
    for long, P, oparam, param, paramstar, paramstar2 in zip(longs_i, (P1, P2, CO),
            (params_i[0], params_i[1], params_i[1]),
            (params_i[0]**2, params_i[1]**2, params_i[0]*params_i[1]),
            (paramsstar_i[0]**2, paramsstar_i[1]**2, paramsstar_i[0]*paramsstar_i[1]),
            (paramsstar2_i[0]**2, paramsstar2_i[1]**2, paramsstar2_i[0]*paramsstar2_i[1])):
        # Initial message
        # Try FFT in just zonal dimension, and 2D FFT
        # Make sure in power sum, to weight Nyquist frequency one half as much as others
        fft_zonal = np.abs(np.fft.rfft(oparam, axis=3)[...,1:]/param.shape[3])**2
        fft_zonal[...,:-1] *= 2 # Nyquist only appears once in full FFT, so only weight once
        fft_2d = np.abs(np.fft.rfft2(oparam, axes=(0,3))[1:,:,:,1:]/param.shape[3]/param.shape[0])**2
        fft_2d[...,:-1] *= 2
        fft_2d_alt = np.abs(np.fft.fft2(oparam, axes=(0,3))[1:,:,:,1:]/param.shape[3]/param.shape[0])**2
