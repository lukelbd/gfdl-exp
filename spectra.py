...
ke_tropic = np.sqrt((utropic - utropic.mean(axis=3, keepdims=True))**2 +
                    (vtropic - vtropic.mean(axis=3, keepdims=True))**2)[:,0,:,:]
ke_clinic = np.sqrt((uclinic - uclinic.mean(axis=3, keepdims=True))**2 +
...
for flux in ('ehf','emf','ke','ke_tropic','ke_clinic'):
    ...
    if params_i[0].ndim==3:
        ...
        axes = (0,3)
        dims = ('f','plev','lat','k')
    ff, kk, spectrum, *powers = \
        climpy.power2d(*params[flux], dx=dt, dy=dlon,
            axes=axes, detrend='constant', wintype=wintype, # can decide on best windowing practice when averaging blocks of days!
    ...
    if out is None:
        ...
        out = xr.Dataset({}, coords={'f':f, 'plev':plev, 'lat':lat, 'k':k}, attrs=data.attrs)
    ...
    if powers: # non-empty, i.e. we got a *co*-spectrum
        ...
        out[shorts_i[2]] = xr.Variable(dims, spectrum, {'long_name':long, 'units':units_i[2]})
    else:
        # We just got a power spectrum; save it
        spectrum = spectrum.squeeze()
        long = f'{longs_i[0]} power spectrum'
        out[shorts_i[0]] = xr.Variable(dims, spectrum, {'long_name':long, 'units':units_i[0]})
